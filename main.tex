...
% Packages (LIPIcs discourages custom packages; these are allowed)
...
Both Ligra and Ligra+ operate on directed graphs $G=(V,E)$ where vertices are numbered $0,\dots,|V|-1$. For each vertex $v$, the tools store outgoing neighbors $N^+(v)$ and incoming neighbors $N^{-}(v)$ alongside their degrees~\cite{shun2013ligra}. Computation uses fork--join parallelism with atomic instructions such as compare-and-swap (CAS) ensuring correctness under concurrent writes.
...
Ligra exposes two core parallel primitives:

\begin{itemize}
    \item \textbf{VERTEXMAP(U, F):} Applies a Boolean function $F$ to each $u \in U$ and returns those for which $F$ succeeds.
    \item \textbf{EDGEMAP(G, U, F, C):} Applies an update function $F$ to edges leaving vertices in $U$, generating candidate vertices, which are filtered by condition $C$.
\end{itemize}
...
Ligra aims to provide a lightweight shared-memory alternative to distributed frameworks. Systems like Pregel~\cite{malewicz2010pregel} or GraphLab~\cite{low2010graphlab} incur synchronization and communication overhead, whereas Ligra is optimized for uniform memory access with fast, shared RAM.

\subsection{Core Techniques}
Ligra's performance hinges on adaptively switching between sparse and dense modes when frontier size crosses a threshold proportional to $|E|$~\cite{shun2013ligra}. Sparse traversal examines only outgoing edges from active vertices; dense traversal checks all vertices using incoming edges.
